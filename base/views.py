    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if contours:
                ...
                context = {
                    'form': form,
                    'result_image': image_base64  # Pass base64 string to template
                }
                return render(request, 'base/upload.html', context)

            else:
                fs.delete(temp_image_path)
                return JsonResponse({'success': False, 'message': 'No contours found.'})

        # If form is not valid
        context = {'form': form, 'result_image': None}
        return render(request, 'base/upload.html', context)
    
    # Handle GET request or unexpected cases
